...
func (a *App) addIngress(cfg config.ReceiversConfig, tenantMap map[string]string, tenantLimits map[string]config.TenantLimit, journalPath string, journalMaxBytes int64, traceActive bool, spanLimit int) error {
	grpcAddr, httpAddr := "", ""
	security := otlprecv.SecurityConfig{}
	security.TenantMap = tenantMap
	security.JournalPath = journalPath
	...
	for name, limit := range tenantLimits {
		security.TenantLimits[name] = otlprecv.TenantLimit{MaxItems: limit.MaxItems, MaxBytes: limit.MaxBytes, MaxConcurrent: limit.MaxConcurrent, MaxRequestsPerSecond: limit.MaxRequestsPerSecond, MaxLogRecordBytes: limit.MaxLogRecordBytes, MaxLogAttributes: limit.MaxLogAttributes, MaxLogAttributeKeys: limit.MaxLogAttributeKeys, MaxMetricAttributes: limit.MaxMetricAttributes, MaxMetricAttributeKeys: limit.MaxMetricAttributeKeys, MaxMetricSeries: limit.MaxMetricSeries}
	}
	if cfg.OTLPGRPC != nil {
...
func (a *App) selfObsMux(p *pipeline.Pipeline[model.Batch]) http.Handler {
	...
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, _ *http.Request) {
		...
		if a.ingress != nil {
			...
			metricLimitRejected := a.ingress.MetricLimitRejected()
			...
			_, _ = fmt.Fprintf(w, "# TYPE coral_otlp_metric_limit_rejected counter\ncoral_otlp_metric_limit_rejected %d\n", metricLimitRejected)
