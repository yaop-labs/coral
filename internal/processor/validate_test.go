...
func TestValidateProcessor_CredsFilter(t *testing.T) {
	...
	cred := model.Span{
		Name:  "op",
		Attrs: []model.Attribute{model.StringAttr("db.password", "secret")},
	}
	...
	if len(got.Spans) != 1 || got.Spans[0].Name != "clean" {
		t.Errorf("expected 1 clean span, got %d", len(got.Spans))
	}
}
