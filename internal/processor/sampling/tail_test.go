...
func TestTailSampler_MaxTracesKeepsByteAccounting(t *testing.T) {
	ts := NewTail(time.Hour, 1, 1, nil, func(context.Context, model.Batch) error { return nil }, 1<<20)
	ctx := context.Background()
	first := traceSpan(1, 1, model.StatusOK)
	second := traceSpan(2, 2, model.StatusOK)
	if _, err := ts.Process(ctx, model.Batch{Spans: []model.Span{first, second}}); err != nil {
		t.Fatal(err)
	}
	ts.mu.Lock()
	defer ts.mu.Unlock()
...
func TestTailSampler_StatsAreBounded(t *testing.T) {
	ts := NewTail(time.Hour, 10, 1, nil, func(context.Context, model.Batch) error { return nil }, 1<<20)
	if _, err := ts.Process(context.Background(), model.Batch{Spans: []model.Span{traceSpan(11, 1, model.StatusOK)}}); err != nil {
		t.Fatal(err)
	}
	traces, bytes := ts.Stats()
	if traces != 1 || bytes <= 0 {
		t.Fatalf("stats = traces %d bytes %d", traces, bytes)
	}
	_ = ts.Close()
	traces, bytes = ts.Stats()
	if traces != 0 || bytes != 0 {
		t.Fatalf("closed stats = traces %d bytes %d", traces, bytes)
	}
}
