...
func (e *FathomExporter) Export(ctx context.Context, b Batch) error {
	...
	return e.retry.Do(ctx, func(ctx context.Context) error {
		return post(ctx, e.client, e.url, "fathom logs", body, b.Tenant)
	})
}
...
func post(ctx context.Context, client *http.Client, url, who string, body []byte, tenant string) error {
	...
	if partial := response.GetPartialSuccess(); partial != nil && partial.GetRejectedLogRecords() > 0 {
		return backoff.Permanent(fmt.Errorf(
			"%s: partial success rejected_log_records=%d: %s",
			who, partial.GetRejectedLogRecords(), partial.GetErrorMessage(),
		))
	}
	return nil
}
