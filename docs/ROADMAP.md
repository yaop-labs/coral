...
race-tested lock ownership, and durable delivery-identity recovery from journal
v3 are covered.
The v1/v2 journal formats remain readable.
...
configured per-record byte/attribute limits, bounded distinct attribute-key
cardinality, configured redaction/privacy, tenant routing, and Amber storage
contract. Define stable platform query expectations outside Coral.
...
attribute-key/descriptor admission, permanent pre-sink rejection, and
dedicated limit observability are implemented. Full per-series cardinality,
...
and thread-safe pending stats are covered. Tenant-aware trace keying, restart
